...
function value(e) {
  ...
  if (91 === s.charCodeAt(u)) {
    ...
    if (null != (i = n[f.Const])) {
      ...
      return {
        kind: "StringValue",
        value: m.test(i) ? JSON.parse(i) : i.slice(1, -1),
        block: !1
      };
...
var y = o((async function* _runPersisted(o) {
  ...
  for (var m of f) {
    ...
    var S = n(m, v);
    for (var k of S) {
      var y = c.getSourcePosition(m, k.node.getStart());
      g = y.fileName;
      if (!s.has(k.schema)) {
        p.push({
      ...
      if (b && (e.isCallExpression(b) || e.isIdentifier(b))) {
        A = r(b, m.fileName, v).node;
        w = b;
      } else if (C && e.isTypeQueryNode(C)) {
        A = t(C, m.fileName, v).node;
        w = C;
      }
      if (!A) {
        p.push({
