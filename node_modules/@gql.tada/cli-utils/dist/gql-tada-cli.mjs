...
import m, { stdin as g, stdout as E } from "node:process";

import { WriteStream as w } from "node:tty";

import { Buffer as b } from "node:buffer";

import B, { ChildProcess as S } from "node:child_process";
...
import M from "node:url";

import U, { constants as _ } from "node:os";

import { createWriteStream as G, createReadStream as V } from "node:fs";

import { setTimeout as H } from "node:timers/promises";

import K from "stream";

import { debuglog as Y } from "node:util";
...
var Q = /^(--[a-z]+(?:-[a-z]+)*|-[a-zA-Z]+)$/;

var ee = /^-[a-zA-Z]{2,}$/;

var re = /^([^=]+)=([\s\S]*)$/;

var ne = "1" === process.env.DEBUG_CLI;
...
var whileRunning = e => `While running ${e.filter((e => e !== z.EndOfInput && e !== z.EndOfPartialInput)).map((e => {
...
var ie = Array(80).fill("━");

for (var ae = 0; ae <= 24; ++ae) {
  ie[ie.length - ae] = `[38;5;${232 + ae}m━`;
}

var oe = {
  header: e => `[1m━━━ ${e}${e.length < 75 ? ` ${ie.slice(e.length + 5).join("")}` : ":"}[0m`,
  bold: e => `[1m${e}[22m`,
...
var ce = Symbol("clipanion/isOption");
...
function runMachineInternal(e, t, r = !1) {
  ...
  var i = [ z.StartOfInput, ...t ];
  for (var a = 0; a < i.length; ++a) {
    var s = i[a];
    var D = s === z.EndOfInput || s === z.EndOfPartialInput;
    var c = a - 1;
    ...
    for (var {node: d, state: f} of n) {
      ...
      if (!r || a < i.length - 1 || h) {
        if (h) {
          ...
          for (var {to: g, reducer: E} of m) {
            l.push({
              node: g,
              state: void 0 !== E ? execute(fe, E, f, s, c) : f
            });
        ...
        for (var b of Object.keys(p.statics)) {
          ...
          if (s === b) {
            for (var {to: B, reducer: S} of p.statics[b]) {
              l.push({
                node: B,
                state: void 0 !== S ? execute(fe, S, f, s, c) : f
              });
      ...
      if (!D) {
        for (var [k, {to: I, reducer: M}] of p.dynamics) {
          if (execute(de, k, f, s, c)) {
            l.push({
              node: I,
              state: void 0 !== M ? execute(fe, M, f, s, c) : f
            });
    ...
    if (0 === l.length) {
      throw new UnknownSyntaxError(t, n.filter((({node: e}) => e !== J.ErrorNode)).map((({state: e}) => ({
        usage: e.candidateUsage,
        reason: null
      }))));
    }
    if (l.every((({node: e}) => e === J.ErrorNode))) {
...
function runMachine(e, t, {endToken: r = z.EndOfInput} = {}) {
  return function selectBestState(e, t) {
    var r = t.filter((e => null !== e.selectedIndex));
    if (0 === r.length) {
      throw new Error;
    }
    var n = r.filter((e => e.selectedIndex === Z || e.requiredOptions.every((t => t.some((t => e.options.find((e => e.name === t))))))));
    if (0 === n.length) {
...
function isTerminalNode(e) {
  return e === J.SuccessNode || e === J.ErrorNode;
}

function cloneTransition(e, t = 0) {
  return {
    to: !isTerminalNode(e.to) ? e.to >= J.CustomNode ? e.to + t - J.CustomNode + 1 : e.to + t : e.to,
    reducer: e.reducer
  };
}
...
var de = {
  ...
  isBatchOption: (e, t, r, n) => !e.ignoreOptions && ee.test(t) && [ ...t.slice(1) ].every((e => n.has(`-${e}`))),
  isBoundOption: (e, t, r, n, i) => {
    var a = t.match(re);
    return !e.ignoreOptions && !!a && Q.test(a[1]) && n.has(a[1]) && i.filter((e => e.nameSet.includes(a[1]))).every((e => e.allowBinding));
  },
  isNegatedOption: (e, t, r, n) => !e.ignoreOptions && t === `--no-${n.slice(2)}`,
  isHelp: (e, t) => !e.ignoreOptions && X.test(t),
  isUnsupportedOption: (e, t, r, n) => !e.ignoreOptions && t.startsWith("-") && Q.test(t) && !n.has(t),
  isInvalidOption: (e, t) => !e.ignoreOptions && t.startsWith("-") && !Q.test(t)
};
...
var fe = {
  ...
  setError: (e, t, r, n) => {
    if (t === z.EndOfInput || t === z.EndOfPartialInput) {
      return {
        ...e,
        errorMessage: `${n}.`
      };
...
class CommandBuilder {
  ...
  addPositional({name: e = "arg", required: t = !0} = {}) {
    ...
    } else if (this.arity.extra !== pe && 0 === this.arity.extra.length) {
  ...
  compile() {
    ...
    registerStatic(e, J.InitialNode, z.StartOfInput, t, [ "setCandidateState", {
      candidateUsage: r,
      requiredOptions: n
    } ]);
    ...
    for (var s of a) {
      ...
      for (var l = 0; l < s.length; ++l) {
        ...
        if (l + 1 < s.length) {
          ...
          registerStatic(e, f, z.EndOfInput, J.SuccessNode, [ "setSelectedIndex", Z ]);
        }
      }
      if (this.arity.leading.length > 0 || !this.arity.proxy) {
        ...
        registerStatic(e, p, z.EndOfInput, J.SuccessNode, [ "setSelectedIndex", Z ]);
        this.registerOptions(e, D);
      }
      if (this.arity.leading.length > 0) {
        registerStatic(e, D, z.EndOfInput, J.ErrorNode, [ "setError", "Not enough positional arguments" ]);
        registerStatic(e, D, z.EndOfPartialInput, J.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
      }
      var h = D;
      for (var m = 0; m < this.arity.leading.length; ++m) {
        ...
        if (this.arity.trailing.length > 0 || m + 1 !== this.arity.leading.length) {
          registerStatic(e, g, z.EndOfInput, J.ErrorNode, [ "setError", "Not enough positional arguments" ]);
          registerStatic(e, g, z.EndOfPartialInput, J.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
        }
        registerDynamic(e, h, "isNotOptionLike", g, "pushPositional");
        h = g;
      }
      var E = h;
      if (this.arity.extra === pe || this.arity.extra.length > 0) {
        var w = injectNode(e, {
          dynamics: [],
          shortcuts: [],
          statics: {}
      ...
      if (this.arity.trailing.length > 0) {
        registerStatic(e, E, z.EndOfInput, J.ErrorNode, [ "setError", "Not enough positional arguments" ]);
        registerStatic(e, E, z.EndOfPartialInput, J.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
      }
      var O = E;
      for (var k = 0; k < this.arity.trailing.length; ++k) {
        ...
        if (k + 1 < this.arity.trailing.length) {
          registerStatic(e, I, z.EndOfInput, J.ErrorNode, [ "setError", "Not enough positional arguments" ]);
          registerStatic(e, I, z.EndOfPartialInput, J.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
        }
        registerDynamic(e, O, "isNotOptionLike", I, "pushPositional");
        O = I;
      }
      registerDynamic(e, O, i, J.ErrorNode, [ "setError", "Extraneous positional argument" ]);
      registerStatic(e, O, z.EndOfInput, J.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
      registerStatic(e, O, z.EndOfPartialInput, J.SuccessNode, [ "setSelectedIndex", this.cliIndex ]);
    }
    return {
      machine: e,
  ...
  registerOptions(e, t) {
    ...
    registerDynamic(e, t, [ "isUnsupportedOption", this.allOptionNames ], J.ErrorNode, [ "setError", "Unsupported option name" ]);
    registerDynamic(e, t, [ "isInvalidOption" ], J.ErrorNode, [ "setError", "Invalid option name" ]);
    for (var r of this.options) {
      if (0 === r.arity) {
        ...
        for (var s = 0; s < r.arity; ++s) {
          ...
          registerStatic(e, i, z.EndOfInput, J.ErrorNode, "setOptionArityError");
          registerStatic(e, i, z.EndOfPartialInput, J.ErrorNode, "setOptionArityError");
          registerDynamic(e, i, "isOptionLike", J.ErrorNode, "setOptionArityError");
  ...
  compile() {
    ...
    var a = function makeAnyOfMachine(e) {
      ...
      for (var i of e) {
        ...
        n += i.nodes.length - J.CustomNode + 1;
      }
      for (var s of r) {
        registerShortcut(t, J.InitialNode, s);
      }
      return t;
    }(e);
    !function simplifyMachine(e) {
      ...
      process(J.InitialNode);
    }(a);
    return {
      machine: a,
      contexts: t,
      process: (e, {partial: t} = {}) => runMachine(a, e, {
        endToken: t ? z.EndOfPartialInput : z.EndOfInput
      })
    };
  }
}
...
var he = Symbol("clipanion/errorCommand");
...
class Cli {
  ...
  error(e, t) {
    var r;
    var {colored: n, command: i = (null !== (r = e[he]) && void 0 !== r ? r : null)} = void 0 === t ? {} : t;
  ...
  format(e) {
    var t;
    return (null !== (t = null != e ? e : this.enableColors) && void 0 !== t ? t : Cli.defaultContext.colorDepth > 1) ? oe : De;
  }
...
var me = function fromIterable(e) {
...
function cmd(e, t, r) {
  ...
  var n = we;
  if (e === ge.SetPrivateMode) {
    n += "?";
    e = ge.SetMode;
  } else if (e === ge.UnsetPrivateMode) {
    n += "?";
    e = ge.UnsetMode;
  }
  if (Array.isArray(t)) {
...
var be = /([\x1B\x9B][[()#;?]*(?:[0-9]{1,4}(?:;[0-9]{0,4})*)?[0-9A-ORZcf-nqry=><])/g;

var stripAnsi = e => e.replace(be, "");
...
function compose(e) {
  var t = cmd(ge.Style, [ ye.Reset, ye.NoInvert ]);
  ...
  return pipe(r, concatMap((e => {
    ...
    return pipe(merge([ pipe(n, takeLast(1), map((e => "string" == typeof e && !e.endsWith("\n") ? "" : e))), n ]), function scan(e, t) {
      ...
      if (t) {
        return cmd(ge.PrevLine, t) + cmd(ge.DeleteLines, t + 1);
      } else if (stripAnsi(e)) {
        return cmd(ge.EraseLine, Ce.Backward) + cmd(ge.ToColumn, 1);
      } else {
        return "";
      }
...
var Oe = !!process.env.GITHUB_ACTIONS;
...
function initTTY(e = {}) {
  ...
  var s = pipe(function fromReadStream(e) {
    ...
    _setColor(t && a || i);
    if (t) {
      n.write(cmd(ge.UnsetPrivateMode, Ee.ShowCursor));
    }
  })), onEnd((function _end() {
    if (t) {
      n.write(cmd(ge.Reset) + cmd(ge.ResetPrivateMode) + cmd(ge.SetPrivateMode, Ee.ShowCursor) + "\n");
    }
  })), share);
  ...
  return {
    ...
    mode: function mode(...e) {
      if (t) {
        ...
        if (r.length) {
          n.write(cmd(ge.SetMode, r));
        }
        if (i.length) {
          n.write(cmd(ge.SetPrivateMode, i));
        }
      }
    },
    modeOff: function modeOff(...e) {
      if (t) {
        ...
        if (r.length) {
          n.write(cmd(ge.UnsetMode, r));
        }
        if (i.length) {
          n.write(cmd(ge.UnsetPrivateMode, i));
        }
      }
    }
  };
...
function code(e) {
  return text`${cmd(ge.Style, ye.Underline)}${e}${cmd(ge.Style, ye.NoUnderline)}`;
}

function bold(e) {
  return text`${cmd(ge.Style, ye.Bold)}${e}${cmd(ge.Style, ye.Normal)}`;
}

function hint(e) {
  return text([ cmd(ge.Style, ye.BrightBlack), `${Se.BottomLeft} `, cmd(ge.Style, ye.BrightBlue), `${xe.Info} `, cmd(ge.Style, ye.Blue), indent(e, "    ") ]);
}

function errorMessage$1(e) {
  return error([ "\n", cmd(ge.Style, [ ye.Red, ye.Invert ]), ` ${xe.Warning} Error `, cmd(ge.Style, ye.NoInvert), `\n${e.trim()}\n` ]);
}
...
function externalError(e, t) {
  ...
  return error([ "\n", cmd(ge.Style, [ ye.Red, ye.Invert ]), ` ${xe.Warning} ${r} `, cmd(ge.Style, ye.NoInvert), `\n${e.trim()}\n`, cmd(ge.Style, ye.BrightBlack), `${Se.BottomLeft} `, indent(n, "  ") ]);
}
...
async function* run$6(t, D) {
  ...
  yield function summary$1(e) {
    var t = text([ cmd(ge.Style, ye.BrightGreen), `${xe.Tick} Introspection output was generated successfully\n` ]);
    if (e) {
...
function warningFile$1(t) {
  var r = e.relative(Te, t);
  if (!r.startsWith("..")) {
    t = r;
  }
  return text([ cmd(ge.Style, ye.Underline), t, cmd(ge.Style, ye.NoUnderline), "\n" ]);
}
...
function runningPersisted(e, t) {
  var r = e ? t ? `(${e}/${t})` : `(${e})` : "";
  return pipe(interval(150), map((e => text([ cmd(ge.Style, ye.Magenta), $e[e % $e.length], " ", cmd(ge.Style, ye.Foreground), `Scanning files${Be.Ellipsis} `, cmd(ge.Style, ye.BrightBlack), r ]))));
}
...
async function* run$5(t, i) {
    ...
    for await (var m of d) {
      ...
      if (f += m.warnings.length) {
        ...
        for (var E of m.warnings) {
          g += (w = E, text([ ke, cmd(ge.Style, ye.BrightBlack), `${w.line}:${w.col}`, Be.Tab, cmd(ge.Style, ye.Foreground), indent(w.message.trim(), text([ ke, Be.Tab ])), Be.Newline ]));
          warningGithub$1(E);
        }
        yield g + "\n";
      }
  ...
  if (f && i.failOnWarn) {
    throw function warningSummary$1(e, t) {
      return error([ cmd(ge.Style, ye.Red), `${xe.Cross} ${e} warnings `, cmd(ge.Style, ye.BrightBlack), `(${t} documents extracted)\n` ]);
    }(f, b);
  } else {
    if (b) {
      try {
    ...
    yield function infoSummary$2(e, t) {
      var r = "";
      if (e) {
        r += text([ cmd(ge.Style, ye.BrightYellow), xe.Warning, ` ${e} warnings\n` ]);
      }
      if (t) {
        r += text([ cmd(ge.Style, ye.BrightGreen), `${xe.Tick} Persisted manifest was generated successfully `, cmd(ge.Style, ye.BrightBlack), `(${t} documents extracted)\n` ]);
      } else {
        r += text([ cmd(ge.Style, ye.Blue), `${xe.Info} No persisted documents were found `, cmd(ge.Style, ye.BrightBlack), "(Persisted manifest was not generated)\n" ]);
      }
      return r;
    }(f, b);
  }
...
async function* run$4(e, a) {
  ...
  yield function summary() {
    return text([ cmd(ge.Style, ye.BrightGreen), `${xe.Tick} Introspection output was generated successfully\n` ]);
  }();
}

var Me = isArray(cascade(isString(), matchesRegExp(/^[\w-]+[ ]*:[ ]*.+/i)));
...
var Re = [ "/* eslint-disable */", "/* prettier-ignore */" ].join("\n") + "\n";
...
async function* run$3(t, i) {
  ...
  if (!i.output && t.pipeTo) {
    ...
    yield function hintMessage(e) {
      return error([ cmd(ge.Style, [ ye.Yellow, ye.Bold ]), `${xe.Warning} Note: `, cmd(ge.Style, ye.Reset), `${e.trim()}\n\n` ]);
  ...
  try {
    for await (var m of d) {
      ...
      if (m.warnings.length) {
        ...
        for (var E of m.warnings) {
          g += (w = E, text([ Le, cmd(ge.Style, ye.BrightBlack), `${w.line}:${w.col}`, Be.Tab, cmd(ge.Style, ye.Foreground), indent(w.message.trim(), text([ Le, Be.Tab ])), Be.Newline ]));
          warningGithub(E);
        }
        yield g + "\n";
      }
  ...
  if (f && i.failOnWarn) {
    throw function warningSummary(e, t) {
      return error([ cmd(ge.Style, ye.Red), `${xe.Cross} ${e} warnings `, cmd(ge.Style, ye.BrightBlack), `(${t} document types cached)\n` ]);
    }(f, b);
  } else {
    try {
      var B = function createCacheContents(e) {
        ...
        return Re + "import type { TadaDocumentNode, $tada } from 'gql.tada';\n\ndeclare module 'gql.tada' {\n interface setupCache {\n" + t + "\n  }\n}\n";
      }(l);
      await writeOutput(c, B);
    } catch (e) {
    ...
    yield function infoSummary$1(e, t) {
      ...
      if (t) {
        r += text([ cmd(ge.Style, ye.BrightGreen), `${xe.Tick} Type cache was generated successfully `, cmd(ge.Style, ye.BrightBlack), `(${t} document types cached)\n` ]);
      } else {
        r += text([ cmd(ge.Style, ye.Blue), `${xe.Info} No documents were found\n` ]);
      }
      return r;
    }(f, b);
  }
...
function diagnosticMessage(e) {
  ...
  return text([ Ge, cmd(ge.Style, ye.BrightBlack), `${e.line}:${e.col}`, Be.Tab, cmd(ge.Style, t), e.severity, Be.Tab, cmd(ge.Style, ye.Foreground), indent(e.message.trim(), text([ Ge, Be.Tab, Be.Tab ])), Be.Newline ]);
}
...
function runningDiagnostics(e, t) {
  var r = e ? t ? `(${e}/${t})` : `(${e})` : "";
  return pipe(interval(150), map((e => text([ cmd(ge.Style, ye.Magenta), $e[e % $e.length], " ", cmd(ge.Style, ye.Foreground), `Checking files${Be.Ellipsis} `, cmd(ge.Style, ye.BrightBlack), r ]))));
}
...
async function* run$2(e, t) {
  ...
  if (t.failOnWarn && D.warn || D.error) {
    throw function problemsSummary(e) {
      ...
      var i = "";
      if (t) {
        i += text([ cmd(ge.Style, ye.Blue), xe.Info, ` ${t} notices\n` ]);
      }
      return error(i += text([ cmd(ge.Style, ye.Red), xe.Cross, ` ${r + n} problems (${r} errors, ${n} warnings)\n` ]));
    }(D);
  } else {
    yield function infoSummary(e) {
      ...
      if (r || n) {
        i += text([ cmd(ge.Style, ye.BrightYellow), xe.Warning, ` ${r + n} problems (${r} errors, ${n} warnings)\n` ]);
      } else {
        i += text([ cmd(ge.Style, ye.BrightGreen), xe.Tick, " No problems found\n" ]);
      }
      return i;
    }(D);
  }
...
var We = new Intl.Collator(0, {
  numeric: 1
}).compare;

function emptyLine() {
  return text([ cmd(ge.Style, ye.BrightBlack), Se.Vertical, "\n" ]);
}

function completedTask(e, t = !1) {
  return text([ emptyLine(), cmd(ge.Style, ye.BrightBlack), t ? Se.BottomLeft : Se.VerticalRight, " ", cmd(ge.Style, ye.Green), xe.TickSwoosh, " ", cmd(ge.Style, ye.Foreground), e, "\n" ]);
}

function failedTask(e) {
  return text([ emptyLine(), cmd(ge.Style, ye.BrightBlack), Se.BottomLeft, " ", cmd(ge.Style, ye.BrightRed), xe.CrossSwoosh, " ", cmd(ge.Style, ye.Foreground), e, "\n" ]);
}

function runningTask(e) {
  return pipe(interval(150), map((t => text([ emptyLine(), cmd(ge.Style, ye.Magenta), Ae[t % Ae.length], " ", cmd(ge.Style, ye.Foreground), e.trim() ]))));
}
...
var semiverComply = (e, t) => {
  ...
  return r ? function semiver(e, t, r) {
    e = e.split(".");
    t = t.split(".");
    return We(e[0], t[0]) || We(e[1], t[1]) || (t[2] = t.slice(2).join("."), (r = /[.-]/.test(e[2] = e.slice(2).join("."))) == /[.-]/.test(t[2]) ? We(e[2], t[2]) : r ? -1 : 1);
  }(r[0], t) >= 0 : !1;
};
...
var qe = "4.1.0", He = "1.0.0";

async function* run$1() {
  yield function title(e, t) {
    var r = text([ cmd(ge.Style, ye.BrightBlack), Se.TopLeft, " ", cmd(ge.Style, [ ye.Magenta, ye.Invert ]), ` ${e.trim()} `, cmd(ge.Style, [ ye.NoInvert ]), "\n" ]);
    if (t) {
      r += text([ cmd(ge.Style, ye.BrightBlack), Se.Vertical, ` ${t}\n` ]);
    }
    return r;
  }(Ve.TITLE, Ve.DESCRIPTION);
  yield runningTask(Ve.CHECK_TS_VERSION);
  await delay();
  var e = process.cwd();
  ...
  try {
    ...
    yield failedTask(Ve.CHECK_TS_VERSION);
  ...
  } else if (!semiverComply(d[1], qe)) {
    yield failedTask(Ve.CHECK_TS_VERSION);
    throw errorMessage(`The version of ${code("typescript")} in your dependencies is out of date.\n` + hint(`${code("gql.tada")} requires at least ${bold(qe)}`));
  }
  yield completedTask(Ve.CHECK_TS_VERSION);
  yield runningTask(Ve.CHECK_DEPENDENCIES);
  await delay();
  var f = c.find((e => "@0no-co/graphqlsp" === e[0]));
  if (!f) {
    yield failedTask(Ve.CHECK_DEPENDENCIES);
  ...
  if (!p) {
    ...
    yield failedTask(Ve.CHECK_DEPENDENCIES);
    throw errorMessage(`The version of ${code("gql.tada")} in your dependencies is out of date.\n` + hint(`It's recommended to upgrade ${code("gql.tada")} to at least ${bold(He)}`));
  }
  yield completedTask(Ve.CHECK_DEPENDENCIES);
  yield runningTask(Ve.CHECK_TSCONFIG);
  await delay();
  var h;
  try {
    h = await r();
  } catch (e) {
    yield failedTask(Ve.CHECK_TSCONFIG);
  ...
  yield completedTask(Ve.CHECK_TSCONFIG);
  yield runningTask(Ve.CHECK_SCHEMA);
  await delay();
  var g = i({
    origin: m.schema,
  ...
  yield completedTask(Ve.CHECK_SCHEMA, !0);
  await delay();
  yield function success() {
    return text([ "\n", cmd(ge.Style, [ ye.Green, ye.Invert ]), " Done ", cmd(ge.Style, ye.NoInvert), Be.Space, "You are all set and ready to go.\n" ]);
  }();
}
...
var Ye = {
  ...
  forward: (e = 1) => `${Ke}${e}C`,
  backward: (e = 1) => `${Ke}${e}D`,
  nextLine: (e = 1) => `${Ke}E`.repeat(e),
  prevLine: (e = 1) => `${Ke}F`.repeat(e),
  left: `${Ke}G`,
  hide: `${Ke}?25l`,
  show: `${Ke}?25h`,
  save: "7",
...
var ze = {
  screen: `${Ke}2J`,
  up: (e = 1) => `${Ke}1J`.repeat(e),
  down: (e = 1) => `${Ke}J`.repeat(e),
  line: `${Ke}2K`,
  lineEnd: `${Ke}K`,
  lineStart: `${Ke}1K`,
  lines(e) {
...
var Je = {
  cursor: Ye,
  scroll: {
    up: (e = 1) => `${Ke}S`.repeat(e),
    down: (e = 1) => `${Ke}T`.repeat(e)
  },
  erase: ze,
  beep: ""
};
...
Ze.exports.createColors = create;

var Qe = getDefaultExportFromCjs(Ze.exports);
...
!function(e) {
  var t = {};
  et.exports = t, t.eastAsianWidth = function(e) {
...
var T = (e = 0) => t => `[${t + e}m`, P = (e = 0) => t => `[${38 + e};5;${t}m`, W = (e = 0) => (t, r, n) => `[${38 + e};2;${t};${r};${n}m`, nt = {
  modifier: {
    reset: [ 0, 0 ],
    bold: [ 1, 22 ],
    dim: [ 2, 22 ],
...
Object.keys(nt.modifier);

Object.keys(nt.color), Object.keys(nt.bgColor);

var it = function tD() {
  var e = new Map;
  for (var [t, r] of Object.entries(nt)) {
    for (var [n, i] of Object.entries(r)) {
      nt[n] = {
        open: `[${i[0]}m`,
        close: `[${i[1]}m`
      }, r[n] = nt[n], e.set(i[0], i[1]);
    }
    Object.defineProperty(nt, t, {
      value: r,
      enumerable: !1
    });
  }
  return Object.defineProperty(nt, "codes", {
    value: e,
    enumerable: !1
  }), nt.color.close = "[39m", nt.bgColor.close = "[49m", nt.color.ansi = T(), nt.color.ansi256 = P(), 
  nt.color.ansi16m = W(), nt.bgColor.ansi = T(10), nt.bgColor.ansi256 = P(10), nt.bgColor.ansi16m = W(10), 
  Object.defineProperties(nt, {
    ...
    hexToAnsi256: {
      value: e => nt.rgbToAnsi256(...nt.hexToRgb(e)),
      enumerable: !1
    },
    ansi256ToAnsi: {
      value: e => {
    ...
    rgbToAnsi: {
      value: (e, t, r) => nt.ansi256ToAnsi(nt.rgbToAnsi256(e, t, r)),
      enumerable: !1
    },
    hexToAnsi: {
      value: e => nt.ansi256ToAnsi(nt.hexToAnsi256(e)),
      enumerable: !1
    }
  }), nt;
}(), ut = new Set([ "", "" ]), at = "]8;;", N = e => `${ut.values().next().value}[${e}m`, L = e => `${ut.values().next().value}${at}${e}`, y$1 = (e, t, r) => {
  var n = [ ...t ];
  var i = !1, a = !1, s = A(S$1(e[e.length - 1]));
  for (var [D, c] of n.entries()) {
    var l = A(c);
    if (s + l <= r ? e[e.length - 1] += c : (e.push(c), s = 0), ut.has(c) && (i = !0, 
    a = n.slice(D + 1).join("").startsWith(at)), i) {
      a ? "" === c && (i = !1, a = !1) : "m" === c && (i = !1);
      continue;
    }
  ...
  for (var [h, m] of p.entries()) {
    if (a += m, ut.has(m)) {
      var {groups: g} = new RegExp(`(?:\\[(?<code>\\d+)m|\\${at}(?<uri>.*))`).exec(p.slice(h).join("")) || {
        groups: {}
      };
      if (void 0 !== g.code) {
...
var ot = Object.defineProperty, a$1 = (e, t, r) => (((e, t, r) => {
  t in e ? ot(e, t, {
    enumerable: !0,
    configurable: !0,
    writable: !0,
...
var Dt = new Map([ [ "k", "up" ], [ "j", "down" ], [ "h", "left" ], [ "l", "right" ] ]), ct = new Set([ "up", "down", "left", "right", "space", "enter" ]);

let lt = class x {
  constructor({render: e, input: t = g, output: r = E, ...n}, i = !0) {
  ...
  prompt() {
    ...
    this.render(), new Promise(((e, t) => {
      this.once("submit", (() => {
        this.output.write(Je.cursor.show), this.output.off("resize", this.render), v(this.input, !1), 
        e(this.value);
      })), this.once("cancel", (() => {
        this.output.write(Je.cursor.show), this.output.off("resize", this.render), v(this.input, !1), 
        e(st);
  ...
  onKeypress(e, t) {
    if ("error" === this.state && (this.state = "active"), t?.name && !this._track && Dt.has(t.name) && this.emit("cursor", Dt.get(t.name)), 
    t?.name && ct.has(t.name) && this.emit("cursor", t.name), e && ("y" === e.toLowerCase() || "n" === e.toLowerCase()) && this.emit("confirm", "y" === e.toLowerCase()), 
  ...
  restoreCursor() {
    ...
    this.output.write(Je.cursor.move(-999, -1 * e));
  }
  render() {
    ...
    if (e !== this._prevFrame) {
      if ("initial" === this.state) {
        this.output.write(Je.cursor.hide);
      } else {
        ...
        if (this.restoreCursor(), t && 1 === t?.length) {
          var r = t[0];
          this.output.write(Je.cursor.move(0, r)), this.output.write(Je.erase.lines(1));
          var n = e.split("\n");
          this.output.write(n[r]), this._prevFrame = e, this.output.write(Je.cursor.move(0, n.length - r - 1));
          return;
        } else if (t && t?.length > 1) {
          var i = t[0];
          this.output.write(Je.cursor.move(0, i)), this.output.write(Je.erase.down());
          var a = e.split("\n").slice(i);
          this.output.write(a.join("\n")), this._prevFrame = e;
          return;
        }
        this.output.write(Je.erase.down());
      }
...
class xD extends lt {
  ...
  constructor(e) {
    super(e, !1), this.value = !!e.initialValue, this.on("value", (() => {
      ...
      this.output.write(Je.cursor.move(0, -1)), this.value = e, this.state = "submit", 
      this.close();
    })), this.on("cursor", (() => {
...
var dt = Object.defineProperty, MD = (e, t, r) => (((e, t, r) => {
  t in e ? dt(e, t, {
    enumerable: !0,
    configurable: !0,
    writable: !0,
  ...
  constructor(e) {
    super(e), MD(this, "valueWithCursor", ""), this.on("finalize", (() => {
      ...
      if (this.cursor >= this.value.length) {
        this.valueWithCursor = `${this.value}${Qe.inverse(Qe.hidden("_"))}`;
      } else {
        var e = this.value.slice(0, this.cursor), t = this.value.slice(this.cursor);
        this.valueWithCursor = `${e}${Qe.inverse(t[0])}${t.slice(1)}`;
      }
    }));
  }
  get cursor() {
...
var ft = globalThis.process.platform.startsWith("win");

var pt = function q() {
  return "win32" !== m.platform ? "linux" !== m.env.TERM : Boolean(m.env.CI) || Boolean(m.env.WT_SESSION) || Boolean(m.env.TERMINUS_SUBLIME) || "{cmd::Cmder}" === m.env.ConEmuTask || "Terminus-Sublime" === m.env.TERM_PROGRAM || "vscode" === m.env.TERM_PROGRAM || "xterm-256color" === m.env.TERM || "alacritty" === m.env.TERM || "JetBrains-JediTerm" === m.env.TERMINAL_EMULATOR;
}(), o = (e, t) => pt ? e : t, ht = o("◆", "*"), mt = o("■", "x"), vt = o("▲", "x"), gt = o("◇", "o"), Ct = o("┌", "T"), Ft = o("│", "|"), Et = o("└", "—"), yt = o("●", ">"), wt = o("○", " "), y = e => {
  switch (e) {
   case "initial":
   case "active":
    return Qe.cyan(ht);
  ...
  render() {
    var t = `${Qe.gray(Ft)}\n${y(this.state)}  ${e.message}\n`, r = e.placeholder ? Qe.inverse(e.placeholder[0]) + Qe.dim(e.placeholder.slice(1)) : Qe.inverse(Qe.hidden("_")), n = this.value ? this.valueWithCursor : r;
    switch (this.state) {
     case "error":
      return `${t.trim()}\n${Qe.yellow(Ft)}  ${n}\n${Qe.yellow(Et)}  ${Qe.yellow(this.error)}\n`;

     case "submit":
      return `${t}${Qe.gray(Ft)}  ${Qe.dim(this.value || e.placeholder)}`;

     case "cancel":
      return `${t}${Qe.gray(Ft)}  ${Qe.strikethrough(Qe.dim(this.value ?? ""))}${this.value?.trim() ? "\n" + Qe.gray(Ft) : ""}`;

     default:
      return `${t}${Qe.cyan(Ft)}  ${n}\n${Qe.cyan(Et)}\n`;
    }
  }
}).prompt(), se = e => {
  ...
  return new xD({
    ...
    render() {
      ...
      switch (this.state) {
       case "submit":
        return `${n}${Qe.gray(Ft)}  ${Qe.dim(i)}`;

       case "cancel":
        return `${n}${Qe.gray(Ft)}  ${Qe.strikethrough(Qe.dim(i))}\n${Qe.gray(Ft)}`;

       default:
        return `${n}${Qe.cyan(Ft)}  ${this.value ? `${Qe.green(yt)} ${t}` : `${Qe.dim(wt)} ${Qe.dim(t)}`} ${Qe.dim("/")} ${this.value ? `${Qe.dim(wt)} ${Qe.dim(r)}` : `${Qe.green(yt)} ${r}`}\n${Qe.cyan(Et)}\n`;
      }
    }
  }).prompt();
}, ue = (e = "") => {
  process.stdout.write(`${Qe.gray(Et)}  ${Qe.red(e)}\n\n`);
};
...
var Tt = "win32" === process.platform || "cygwin" === process.env.OSTYPE || "msys" === process.env.OSTYPE;
...
var getPathInfo = (e, t) => {
  var r = t.colon || It;
  var n = e.match(/\//) || Tt && e.match(/\\/) ? [ "" ] : [ ...Tt ? [ process.cwd() ] : [], ...(t.path || process.env.PATH || "").split(r) ];
  var i = Tt ? t.pathExt || process.env.PATHEXT || ".EXE;.CMD;.BAT;.COM" : "";
  var a = Tt ? i.split(r) : [ "" ];
  if (Tt) {
    if (-1 !== e.indexOf(".") && "" !== a[0]) {
      a.unshift("");
...
Mt.exports = pathKey$1;

Mt.exports.default = pathKey$1;
...
function resolveCommandAttempt(e, t) {
  ...
  if (s) {
    s = jt.resolve(i ? e.options.cwd : "", s);
  }
  return s;
}

var Ut = function resolveCommand$1(e) {
...
_t.command = function escapeCommand(e) {
  return e = e.replace(Gt, "^$1");
};

_t.argument = function escapeArgument(e, t) {
  e = (e = `"${e = (e = (e = `${e}`).replace(/(\\*)"/g, '$1$1\\"')).replace(/(\\*)$/, "$1$1")}"`).replace(Gt, "^$1");
  if (t) {
    e = e.replace(Gt, "^$1");
  }
  return e;
...
var Yt = function readShebang$1(e) {
  var t = Buffer.alloc(150);
  var r;
  try {
    r = Vt.openSync(e, "r");
    Vt.readSync(r, t, 0, 150, 0);
    Vt.closeSync(r);
  } catch (e) {}
  return shebangCommand(t.toString());
};

var zt = "win32" === process.platform;

var Jt = /\.(?:com|exe)$/i;

var Zt = /node_modules[\\/].bin[\\/][^\\/]+\.cmd$/i;
...
function parseNonShell(e) {
  ...
  if (e.options.forceShell || r) {
    var n = Zt.test(t);
    e.command = qt.normalize(e.command);
    e.command = Kt.command(e.command);
    e.args = e.args.map((e => Kt.argument(e, n)));
...
function spawn(e, t, r) {
  var n = tr(e, t, r);
  var i = er.spawn(n.command, n.args, n.options);
  rr.hookChildProcess(i, n);
  return i;
}

bt.exports = spawn;

bt.exports.spawn = spawn;

bt.exports.sync = function spawnSync(e, t, r) {
  var n = tr(e, t, r);
  var i = er.spawnSync(n.command, n.args, n.options);
  i.error = i.error || rr.verifyENOENTSync(i.status, n);
  return i;
};

bt.exports._parse = tr;

bt.exports._enoent = rr;

var nr = getDefaultExportFromCjs(bt.exports);
...
function npmRunPathEnv({env: e = m.env, ...r} = {}) {
  var n = pathKey({
    env: e = {
      ...e
    }
  ...
  e[n] = function npmRunPath(e = {}) {
    var {cwd: r = m.cwd(), path: n = m.env[pathKey()], execPath: i = m.execPath} = e;
    var a;
    var s = i instanceof URL ? M.fileURLToPath(i) : i;
    var D = r instanceof URL ? M.fileURLToPath(r) : r;
    var c = t.resolve(D);
    var l = [];
    while (a !== c) {
...
var ir = Object.getOwnPropertyDescriptor(Function.prototype, "toString");

var ur = Object.getOwnPropertyDescriptor(Function.prototype.toString, "name");
...
function mimicFunction(e, t, {ignoreNonConfigurable: r = !1} = {}) {
  ...
  ((e, t, r) => {
    ...
    Object.defineProperty(i, "name", ur);
...
var getSignals = () => {
  ...
  return [ ...Dr, ...e ].map(normalizeSignal);
};
...
var findSignalByNumber = (e, t) => {
  var r = t.find((({name: t}) => _.signals[t] === e));
  if (void 0 !== r) {
    return r;
  }
...
var makeError = ({stdout: e, stderr: t, all: r, error: n, signal: i, exitCode: a, command: s, escapedCommand: D, timedOut: c, isCanceled: l, killed: d, parsed: {options: {timeout: f, cwd: p = m.cwd()}}}) => {
  var h = void 0 === (i = null === i ? void 0 : i) ? void 0 : cr[i].description;
...
dr.push("SIGHUP", "SIGINT", "SIGTERM");

if ("win32" !== process.platform) {
  dr.push("SIGALRM", "SIGABRT", "SIGVTALRM", "SIGXCPU", "SIGXFSZ", "SIGUSR2", "SIGTRAP", "SIGSYS", "SIGQUIT", "SIGIOT");
}

if ("linux" === process.platform) {
  dr.push("SIGIO", "SIGPOLL", "SIGPWR", "SIGSTKFLT");
}
...
var fr = Symbol.for("signal-exit emitter");

var pr = globalThis;

var hr = Object.defineProperty.bind(Object);
...
var mr = globalThis.process;

var {onExit: vr} = (gr = processOk(mr) ? new class SignalExit extends SignalExitBase {
  #e="win32" === mr.platform ? "SIGINT" : "SIGHUP";
  #t=new Emitter;
  #r;
  #n;
  #i;
...
}(mr) : new class SignalExitFallback extends SignalExitBase {
  onExit() {
    return () => {};
  }
  load() {}
...
var isSigterm = e => e === U.constants.signals.SIGTERM || "string" == typeof e && "SIGTERM" === e.toUpperCase();
...
var pipeToTarget = (e, t, r) => {
  ...
  if (!(e => e instanceof S && "function" == typeof e.then)(r)) {
...
var getChunkType = e => {
  ...
  if (Number.isInteger(e.byteLength) && Number.isInteger(e.byteOffset) && "[object ArrayBuffer]" === Cr.call(e.buffer)) {
    return "typedArray";
  }
  return "others";
};
...
var getNewContentsLength = e => Er ** Math.ceil(Math.log(e) / Math.log(Er));
...
var {PassThrough: br} = K;

var Br = getDefaultExportFromCjs((function() {
  var e = [];
  var t = new br({
    objectMode: !0
  });
...
var getStreamPromise = (e, {encoding: t, buffer: r, maxBuffer: n}) => {
  ...
  if ("utf8" === t || "utf-8" === t) {
    return async function getStreamAsString(e, t) {
      return getStreamContents(e, wr, t);
    }(e, {
      maxBuffer: n
    });
  }
...
var Sr = (async () => {})().constructor.prototype;

var xr = [ "then", "catch", "finally" ].map((e => [ e, Reflect.getOwnPropertyDescriptor(Sr, e) ]));
...
var getEscapedCommand = (e, t) => normalizeArgs(e, t).map((e => (e => {
  if ("string" != typeof e || $r.test(e)) {
    return e;
  }
  return `"${e.replaceAll('"', '\\"')}"`;
})(e))).join(" ");

var Ar = Y("execa").enabled;
...
var logCommand = (e, {verbose: t}) => {
  ...
  m.stderr.write(`[${r = new Date, `${padField(r.getHours(), 2)}:${padField(r.getMinutes(), 2)}:${padField(r.getSeconds(), 2)}.${padField(r.getMilliseconds(), 3)}`}] ${e}\n`);
  var r;
};

var handleArguments = (e, r, n = {}) => {
  var i = nr._parse(e, r, n);
  e = i.command;
  r = i.args;
  (n = {
    maxBuffer: 1e8,
    ...
    extendEnv: !0,
    preferLocal: !1,
    localDir: (n = i.options).cwd || m.cwd(),
    execPath: m.execPath,
    encoding: "utf8",
    reject: !0,
    cleanup: !0,
  ...
  if ("win32" === m.platform && "cmd" === t.basename(e, ".exe")) {
    r.unshift("/q");
  }
  return {
    file: e,
...
var handleOutput = (e, t, r) => {
  if ("string" != typeof t && !b.isBuffer(t)) {
...
function execa(e, t, r) {
  ...
  var s;
  try {
    s = B.spawn(n.file, n.args, n.options);
  } catch (e) {
    var D = new B.ChildProcess;
...
var Or = (() => {
  var e = pt ? [ "◒", "◐", "◓", "◑" ] : [ "•", "o", "O", "0" ], t = pt ? 80 : 120;
  var r, n, i = !1, a = "";
  var u = (e = "", t = 0) => {
    a = e ?? a, i = !1, clearInterval(n);
    var s = 0 === t ? Qe.green(gt) : 1 === t ? Qe.red(mt) : Qe.red(vt);
    process.stdout.write(Je.cursor.move(-999, 0)), process.stdout.write(Je.erase.down(1)), 
  ...
  {
    start: (s = "") => {
      i = !0, r = function WD({input: e = g, output: t = E, overwrite: r = !0, hideCursor: n = !0} = {}) {
        ...
        return n && process.stdout.write(Je.cursor.hide), e.once("keypress", C), () => {
          e.off("keypress", C), n && process.stdout.write(Je.cursor.show), e.isTTY && !ft && e.setRawMode(!1), 
          i.terminal = !1, i.close();
        };
      }(), a = s.replace(/\.+$/, ""), process.stdout.write(`${Qe.gray(Ft)}\n`);
      var D = 0, c = 0;
      n = setInterval((() => {
        var t = Qe.magenta(e[D]), r = ".".repeat(Math.floor(c)).slice(0, 3);
        process.stdout.write(Je.cursor.move(-999, 0)), process.stdout.write(Je.erase.down(1)), 
...
async function run(e) {
  ((e = "") => {
    process.stdout.write(`${Qe.gray(Ct)}  ${e}\n`);
  })("GQL.Tada");
  var r = await question("Where can we get your schema? Point us at an introspection JSON-file, a GraphQL schema file or an endpoint", (async r => {
    try {
      var n = new URL(r);
      Or.start("Validating the URL.");
      try {
        var i = await fetch(n.toString());
        if (!i.ok) {
          Or.stop("Validated the URL.");
          return !!await se({
  ...
  if (i) {
    ...
    Or.stop("Installed packages.");
  } else {
    Or.start("Writing to package.json.");
    try {
      var a = t.resolve(e, "package.json");
      ...
      Or.stop("Written to package.json.");
    } catch (e) {
      Or.stop('Failed to write to package.json, you can try adding "gql.tada" and "@0no-co/graphqlsp" yourself.');
    }
  }
  Or.start("Writing to tsconfig.json.");
  try {
  ...
  Or.stop("Written to tsconfig.json.");
  ((e = "") => {
    process.stdout.write(`${Qe.gray(Ft)}\n${Qe.gray(Et)}  ${e}\n\n`);
  })("Off to the races!");
}
...
var Tr = Object.assign((async function _main() {
  var e = new Cli({
...
export { Tr as default, generateOutput, generatePersisted, generateSchema, generateTurbo };
