...
function advance(e) {
  e.lastIndex = L;
  if (e.test(F)) {
    return F.slice(L, L = e.lastIndex);
  }
}
...
function ignored() {
  for (var e = 0 | F.charCodeAt(L++); 9 === e || 10 === e || 13 === e || 32 === e || 35 === e || 44 === e || 65279 === e; e = 0 | F.charCodeAt(L++)) {
    if (35 === e) {
      while (10 !== (e = F.charCodeAt(L++)) && 13 !== e) {}
    }
  }
  L--;
}
...
function selectionSet() {
  ...
  if (123 === F.charCodeAt(L)) {
    ...
    if (!t.length || 125 !== F.charCodeAt(L++)) {
...
var ae = new Set([ "map", "filter", "forEach", "reduce", "every", "some", "find", "flatMap", "sort" ]);
  ...
  return s = o.flatMap((s => {
    ...
    var u, _loop2 = function() {
      if (!n && (A.isReturnStatement(o) || A.isArrowFunction(o))) {
        ...
        o = o.parent;
      } else if (A.isPropertyAccessExpression(o) && ae.has(o.name.text) && A.isCallExpression(o.parent)) {
...
function getPersistedCodeFixAtPosition(e, t, i) {
  ...
  var d = getSource(i, o);
  var {fragments: v} = findAllCallExpressions(d, i);
  var g = resolveTemplate(p.arguments[0], o, i).combinedText;
  var m = function parse(e, t) {
  ...
  var x = b("sha256").update(g).digest("hex");
  var T = n.arguments[0];
  if (!T) {
    return {
      span: {
      ...
      replacement: `"sha256:${x}")`
    };
  } else if (A.isStringLiteral(T) && T.getText() !== `"sha256:${x}"`) {
    return {
      span: {
        start: T.getStart(),
        length: T.end - T.getStart()
      },
      replacement: `"sha256:${x}"`
    };
  } else if (A.isIdentifier(T)) {
    return {
      span: {
        start: T.getStart(),
        length: T.end - T.getStart()
...
var ne = new Set([ "populate", "client", "_unmask", "_optional", "_relayPagination", "_simplePagination", "_required", "optional", "required", "arguments", "argumentDefinitions", "connection", "refetchable", "relay", "required", "inline" ]);
...
var ue = [ 52001, 52002, 52004, 52003, 52005, se, oe, le ];
  ...
  if (r) {
    var p = findAllPersistedCallExpressions(a).map((t => {
      ...
      if (t.typeArguments) {
        ...
        var {node: u} = getDocumentReferenceFromTypeQuery(l, e, i);
        r = u;
        n = l.getText();
        ...
        var {node: h} = getDocumentReferenceFromDocumentNode(t.arguments[1], e, i);
        r = h;
      ...
      var f = r;
      if (!f || !A.isCallExpression(f) || !A.isNoSubstitutionTemplateLiteral(f.arguments[0])) {
        return {
          category: A.DiagnosticCategory.Warning,
          code: le,
          file: a,
