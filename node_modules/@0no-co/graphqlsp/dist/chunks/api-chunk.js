...
var L = globalThis.AbortController;
...
if (void 0 === L) {
  ...
  var C = "1" !== k.env?.LRU_CACHE_IGNORE_AC_WARNING;
  var warnACPolyfill = () => {
    if (!C) {
      return;
    }
...
class LRUCache {
  ...
  * rvalues() {
  ...
  #A(e) {
    ...
    var t = e;
    return !!t && t instanceof Promise && t.hasOwnProperty("__staleWhileFetching") && t.__abortController instanceof L;
  }
  async fetch(e, t = {}) {
...
var O = getDefaultExportFromCjs(_.exports);
  ...
  return n = o.flatMap((n => {
    ...
    var u, _loop2 = function() {
      if (!a && (exports.ts.isReturnStatement(o) || exports.ts.isArrowFunction(o))) {
        ...
        var p = o.parent;
        var h = p.arguments[0];
        if (h && exports.ts.isIdentifier(h)) {
        ...
        if (h && (exports.ts.isFunctionDeclaration(h) || exports.ts.isFunctionExpression(h) || exports.ts.isArrowFunction(h))) {
          var d = h.parameters[n ? 1 : 0];
          if (d) {
            var v = crawlScope(d.name, l, i, r, s, !0);
            if (exports.ts.isVariableDeclaration(p.parent) && !u) {
              var g = crawlScope(p.parent.name, l, i, r, s, !0);
              v.push(...g);
            }
            return {
              v
        ...
        var x = [ ...l, o.argumentExpression.text ].join(".");
        if (i.find((e => e.startsWith(x)))) {
...
var generateHashForDocument = (e, i, r, s) => {
  if (s) {
    ...
    a.map((e => print(e))).filter(((e, t, i) => i.indexOf(e) === t)).forEach((e => {
      n = `${n}\n\n${e}`;
    }));
    var o = print(parse(n));
    ...
    var l = getSource(e, r);
    var {fragments: u} = findAllCallExpressions(l, e);
    var p = resolveTemplate(i, r, e).combinedText;
    var h = parse(p);
    var f = new Set;
    visit(h, {
      FragmentSpread: e => {
    ...
    while (g = v.shift()) {
      d.add(g);
      var m = u.find((e => e.name.value === g));
      if (!m) {
        e.project.projectService.logger.info(`[GraphQLSP] could not find fragment for spread ${g}!`);
        return;
      }
      visit(m, {
        FragmentSpread: e => {
          if (!d.has(e.name.value)) {
            v.push(e.name.value);
          }
        }
      });
    ...
    return t.createHash("sha256").update(print(parse(c))).digest("hex");
  }
};
