  ...
  WorkspaceChange.prototype.getTextEditChange = function(e) {
    if (R.is(e)) {
      ...
      if (!(n = this._textEditChanges[t.uri])) {
        ...
        n = new O(i, this._changeAnnotations);
...
!function(e) {
  ...
  e.is = function is(e) {
    var t = e;
    return t && Ae.string(t.title) && (void 0 === t.diagnostics || Ae.typedArray(t.diagnostics, b.is)) && (void 0 === t.kind || Ae.string(t.kind)) && (void 0 !== t.edit || void 0 !== t.command) && (void 0 === t.command || S.is(t.command)) && (void 0 === t.isPreferred || Ae.boolean(t.isPreferred)) && (void 0 === t.edit || x.is(t.edit));
  };
}(ue || (ue = {}));
...
function getAutocompleteSuggestions(e, t, n, i, a, o) {
  ...
  var d = (null == o ? void 0 : o.mode) || function getDocumentMode(e, t) {
    if (null == t ? void 0 : t.endsWith(".graphqls")) {
      return Oe.TYPE_SYSTEM;
    }
    return hasTypeSystemDefinitions(e) ? Oe.TYPE_SYSTEM : Oe.EXECUTABLE;
  ...
  if ((null === (s = null == v ? void 0 : v.prevState) || void 0 === s ? void 0 : s.kind) === Le.EXTENSION_DEFINITION && l.name) {
    return hintList$1(u, []);
  }
  ...
  if (f === Le.IMPLEMENTS || f === Le.NAMED_TYPE && (null == v ? void 0 : v.kind) === Le.IMPLEMENTS) {
  ...
  if (f === Le.ARGUMENTS || f === Le.ARGUMENT && 0 === p) {
    var {argDefs: m} = g;
    if (m) {
  ...
  if ((f === Le.OBJECT_VALUE || f === Le.OBJECT_FIELD && 0 === p) && g.objectFieldDefs) {
  ...
  if (f === Le.ENUM_VALUE || f === Le.LIST_VALUE && 1 === p || f === Le.OBJECT_FIELD && 2 === p || f === Le.ARGUMENT && 2 === p) {
  ...
  var y = unwrapType(l);
  if (d === Oe.TYPE_SYSTEM && !y.needsAdvance && f === Le.NAMED_TYPE || f === Le.LIST_TYPE) {
  ...
  if (f === Le.VARIABLE_DEFINITION && 2 === p || f === Le.LIST_TYPE && 1 === p || f === Le.NAMED_TYPE && v && (v.kind === Le.VARIABLE_DEFINITION || v.kind === Le.LIST_TYPE || v.kind === Le.NON_NULL_TYPE)) {
...
function getGraphQLCompletions(t, n, i, a) {
  ...
  var u, l;
  if (e.ts.isCallExpression(c) && o && e.templates.has(c.expression.getText()) && c.arguments.length > 0 && e.ts.isNoSubstitutionTemplateLiteral(c.arguments[0])) {
    var d = getToken(c.arguments[0], n);
    if (!i.current || !d) {
      return;
    }
    u = `${c.arguments[0].getText().slice(1, -1)}\n${e.getAllFragments(t, c, a).map((t => e.print(t))).join("\n")}`;
    l = new Cursor(d.line, d.start - 1);
  } else if (e.ts.isTaggedTemplateExpression(c)) {
    var {template: f, tag: p} = c;
    if (!e.ts.isIdentifier(p) || !e.templates.has(p.text)) {
      return;
    }
    var v = getToken(f, n);
    if (!v || !i.current) {
    ...
    var {combinedText: g, resolvedSpans: m} = e.resolveTemplate(c, t, a);
    var E = m.filter((e => e.original.start < n && e.original.start + e.original.length < n)).reduce(((e, t) => e + (t.lines - 1)), 0);
    v.line = v.line + E;
    u = g;
    l = new Cursor(v.line, v.start - 1);
  } else {
    return;
  }
  var [h, T] = function getSuggestionsInternal(e, t, n) {
...
function create(t) {
  ...
  var o = ((e, t, r) => {
    ...
    (async () => {
      var s = await i.resolveTypeScriptRootDir(e.project.getProjectName()) || n.dirname(e.project.getProjectName());
      var c = e.config.tadaDisablePreprocessing ?? !1;
      var u = e.config.tadaOutputLocation && n.resolve(s, e.config.tadaOutputLocation);
      r("Got root-directory to resolve schema from: " + s);
      ...
      try {
        r(`Loading schema from "${t}"`);
        a = await l.load();
      } catch (e) {
        r(`Failed to load schema: ${e}`);
      }
      if (a) {
        o.current = a && a.schema;
        o.version++;
        if (u) {
          saveTadaIntrospection(a.introspection, u, c, r);
        }
      }
      l.notifyOnUpdate((e => {
        r(`Got schema for origin "${t}"`);
        o.current = (a = e).schema;
        o.version++;
        if (u) {
          saveTadaIntrospection(a.introspection, u, c, r);
        }
  ...
  a.getQuickInfoAtPosition = (n, i) => {
    var r = function getGraphQLQuickInfo(t, n, i, r) {
      ...
      var o = e.getSource(r, t);
      if (!o) {
        return;
      }
      var s = e.findNode(o, n);
      if (!s) {
        return;
      }
      s = a ? e.bubbleUpCallExpression(s) : e.bubbleUpTemplate(s);
      var c, u;
      if (e.ts.isCallExpression(s) && a && e.templates.has(s.expression.getText()) && s.arguments.length > 0 && e.ts.isNoSubstitutionTemplateLiteral(s.arguments[0])) {
        var l = getToken(s.arguments[0], n);
        if (!i.current || !l) {
          return;
        }
        u = s.arguments[0].getText();
        c = new Cursor(l.line, l.start - 1);
      } else if (e.ts.isTaggedTemplateExpression(s)) {
        var {template: d, tag: f} = s;
        if (!e.ts.isIdentifier(f) || !e.templates.has(f.text)) {
          return;
        }
        var p = getToken(d, n);
        if (!p || !i.current) {
        ...
        var {combinedText: v, resolvedSpans: g} = e.resolveTemplate(s, t, r);
        var m = g.filter((e => e.original.start < n && e.original.start + e.original.length < n)).reduce(((e, t) => e + (t.lines - 1)), 0);
        p.line = p.line + m;
        u = v;
      ...
      var E = getHoverInformation(i.current, u, c);
      return {
        ...
        kindModifiers: "text",
        documentation: Array.isArray(E) ? E.map((e => ({
          kind: "text",
          text: e
        }))) : [ {
          kind: "text",
